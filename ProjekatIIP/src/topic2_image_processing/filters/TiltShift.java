		...
		if(ts == 'r') {
			...
			for (int y = 0; y < h; y++) {
				for (int x = 0; x < w; x++) {
					...
					double xx = 2.0*x/w - 1;             // Udaljenost od centra po x-osi, normalizovana na interval (-1,1).
					double yy = 2.0*y/h - 1;             // Udaljenost od centra po x-osi, normalizovana na interval (-1,1).
					
					double d = Math.sqrt(xx*xx + yy*yy); // Udaljenost od centra.
					if (d <= 1) {
						
						r = pr.getColor(x, y).getRed();
						g = pr.getColor(x, y).getGreen();
						b = pr.getColor(x, y).getBlue();
